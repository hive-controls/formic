 ...
 * environment as of 2026-09-02, so this is the whole offline-testable surface).
 */
import { test } from "node:test";
import assert from "node:assert/strict";
import { buildBrowserStackWsEndpoint } from "./browserstack.mts";
...
test("builds a wss://cdp.browserstack.com/playwright endpoint with url-encoded caps", () => {
  ...
  assert.equal(caps.build, "formic-e2e-doctor");
  assert.equal(caps.name, "e2e-doctor run");
});
