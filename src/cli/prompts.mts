/**
 * Interactive prompts for Launchie — three primitives over node:readline.
 *
 * Deliberately duplicated rather than imported: the platform depends on no recipe, and
 * a recipe's own wizard needs the same three primitives. Sixty lines of readline
 * handling is the cheaper half of that trade (same call as status/stream.mts).
 *
 * The interface is always built with `terminal: true` regardless of `PromptIo.isTty` (readline's own
